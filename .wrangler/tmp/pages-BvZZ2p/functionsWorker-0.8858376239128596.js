...
var ProviderError = class extends Error {
  constructor(message2, provider, retryable, status) {
    super(message2);
...
// api/_lib/providers/base.ts
var SYSTEM_INSTRUCTION = `You are StudiByte AI, an intelligent academic assistant built into the StudiByte student productivity app.
...
__name(openAICompatibleChat, "openAICompatibleChat");

// api/_lib/providers/gemini.ts
...
async function call(model, apiKey, contents) {
...
var geminiProvider = {
  name: "gemini",
  generateResponse(messages, env2) {
    if (!env2.GEMINI_API_KEY) throw new ProviderError("gemini not configured", "gemini", false);
    return withTimeout(call(env2.GEMINI_MODEL, env2.GEMINI_API_KEY, toGeminiContents(messages)), PROVIDER_TIMEOUT_MS, "gemini");
  },
  generateTitle(firstMessage, env2) {
    if (!env2.GEMINI_API_KEY) throw new ProviderError("gemini not configured", "gemini", false);
    const prompt = `Summarize this in a short 5-word title, no punctuation: "${firstMessage.slice(0, 500)}"`;
    return withTimeout(
      call(env2.GEMINI_MODEL, env2.GEMINI_API_KEY, [{ role: "user", parts: [{ text: prompt }] }]),
      PROVIDER_TIMEOUT_MS,
      "gemini"
    );
  }
...
// api/_lib/providers/groq.ts
var GROQ_URL = "https://api.groq.com/openai/v1/chat/completions";
var groqProvider = {
  name: "groq",
  generateResponse(messages, env2) {
    if (!env2.GROQ_API_KEY) throw new ProviderError("groq not configured", "groq", false);
    return withTimeout(
      openAICompatibleChat({ url: GROQ_URL, apiKey: env2.GROQ_API_KEY, model: env2.GROQ_MODEL, messages, provider: "groq" }),
      PROVIDER_TIMEOUT_MS,
      "groq"
    );
  },
  generateTitle(firstMessage, env2) {
    if (!env2.GROQ_API_KEY) throw new ProviderError("groq not configured", "groq", false);
    const prompt = [{ role: "user", content: `Summarize this in a short 5-word title, no punctuation: "${firstMessage.slice(0, 500)}"` }];
    return withTimeout(
      openAICompatibleChat({ url: GROQ_URL, apiKey: env2.GROQ_API_KEY, model: env2.GROQ_MODEL, messages: prompt, provider: "groq", maxTokens: 20 }),
      PROVIDER_TIMEOUT_MS,
      "groq"
    );
  }
...
// api/_lib/providers/deepseek.ts
var DEEPSEEK_URL = "https://api.deepseek.com/chat/completions";
var deepseekProvider = {
  name: "deepseek",
  generateResponse(messages, env2) {
    if (!env2.DEEPSEEK_API_KEY) throw new ProviderError("deepseek not configured", "deepseek", false);
    return withTimeout(
      openAICompatibleChat({ url: DEEPSEEK_URL, apiKey: env2.DEEPSEEK_API_KEY, model: env2.DEEPSEEK_MODEL, messages, provider: "deepseek" }),
      PROVIDER_TIMEOUT_MS,
      "deepseek"
    );
  },
  generateTitle(firstMessage, env2) {
    ...
    const prompt = [{ role: "user", content: `Summarize this in a short 5-word title, no punctuation: "${firstMessage.slice(0, 500)}"` }];
    return withTimeout(
      openAICompatibleChat({ url: DEEPSEEK_URL, apiKey: env2.DEEPSEEK_API_KEY, model: env2.DEEPSEEK_MODEL, messages: prompt, provider: "deepseek", maxTokens: 20 }),
      PROVIDER_TIMEOUT_MS,
      "deepseek"
    );
  }
...
// api/_lib/providers/openrouter.ts
...
var openrouterProvider = {
  name: "openrouter",
  generateResponse(messages, env2) {
    if (!env2.OPENROUTER_API_KEY) throw new ProviderError("openrouter not configured", "openrouter", false);
    return withTimeout(
...
__name(isSkippable, "isSkippable");
...
var PROVIDER_ORDER = ["gemini", "groq", "deepseek", "openrouter"];
var SAME_PROVIDER_RETRY_DELAY_MS = 500;
function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}
__name(sleep, "sleep");
async function tryProvider(provider, messages, env2) {
  const start = Date.now();
  try {
    const text = await provider.generateResponse(messages, env2);
...
async function routeGenerateResponse(messages, env2) {
  let lastError = null;
  let fallbackCount = 0;
  for (const name of PROVIDER_ORDER) {
    ...
    try {
      const { text, latencyMs } = await tryProvider(PROVIDERS[name], messages, env2);
...
__name(routeGenerateResponse, "routeGenerateResponse");
...
var AuthError = class extends Error {
  constructor(message2, status = 401) {
    super(message2);
    this.status = status;
    this.name = "AuthError";
  }
...
async function verifyFirebaseToken(request, env2) {
  const header = request.headers.get("Authorization") ?? "";
  const token = header.startsWith("Bearer ") ? header.slice(7) : null;
  if (!token) throw new AuthError("Missing Authorization header");
  if (!env2.FIREBASE_PROJECT_ID) throw new AuthError("Auth not configured", 403);
  try {
...
async function checkRateLimit(uid, env2) {
  const limit = Number(env2.AI_RATE_LIMIT_PER_MINUTE) || 15;
  const windowStart = Math.floor(Date.now() / (WINDOW_SECONDS * 1e3));
  const key = `ratelimit:${uid}:${windowStart}`;
  const current = Number(await env2.RATE_LIMIT_KV.get(key) ?? "0");
  if (current >= limit) throw new RateLimitError(WINDOW_SECONDS);
  await env2.RATE_LIMIT_KV.put(key, String(current + 1), { expirationTtl: WINDOW_SECONDS + 10 });
}
...
function parseMessages(body) {
  ...
  return raw.map((m) => {
    ...
    if (content.length > 32e3) throw new Error("Message is too long");
    return { role, content };
  });
}
...
var onRequestPost = /* @__PURE__ */ __name(async ({ request, env: env2 }) => {
  let uid;
  try {
    uid = await verifyFirebaseToken(request, env2);
  } catch (err) {
    const status = err instanceof AuthError ? err.status : 401;
    return json(status, { error: friendlyError(status) });
  }
  try {
    await checkRateLimit(uid, env2);
  } catch (err) {
  ...
  try {
    const result = await routeGenerateResponse(messages, env2);
...
// ../.wrangler/tmp/pages-BvZZ2p/functionsRoutes-0.5983563433708434.mjs
var routes = [
  {
