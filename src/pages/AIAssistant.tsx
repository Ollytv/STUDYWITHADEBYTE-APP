...
function renderMarkdown(text: string): string {
  return text
    ...
    .replace(/(\|.+\|\n?)+/g, (table) => {
      ...
      let inBody = false;
      rows.forEach((row, i) => {
        if (isHeader(row)) { html += '<tbody>'; inBody = true; return; }
  ...
  return (
    <motion.div
      className={`flex items-center gap-2 px-3 py-2.5 rounded-xl cursor-pointer group transition-colors
        ${active ? 'bg-green-500/15 border border-green-500/20' : 'hover:bg-white/5'}`}
      onClick={onSelect}
      whileTap={{ scale: 0.97 }}
    >
      <MessageSquare size={13} className={active ? 'text-green-400' : 'text-dark-500'} />
      <p className={`flex-1 text-xs font-body truncate ${active ? 'text-white' : 'text-dark-300'}`}>
        {conv.title}
      </p>
      <button
        onClick={e => { e.stopPropagation(); onDelete(); }}
        className="opacity-0 group-hover:opacity-100 p-1 rounded-lg hover:bg-red-500/10 text-dark-500 hover:text-red-400 transition-all"
      >
        <Trash2 size={11} />
      </button>
    </motion.div>
...
export default function AIAssistant() {
  ...
  const upsertConv = useCallback((conv: ChatConversation) => {
    ...
    <div className="flex flex-col min-h-screen bg-dark-950 relative">
      ...
      <AnimatePresence>
        {showSidebar && (
          <>
            ...
            />
            <motion.div
              className="fixed left-0 top-0 bottom-0 z-50 w-72 bg-dark-900 border-r border-white/8 flex flex-col"
            ...
            >
              <div className="p-4 border-b border-white/8 flex items-center justify-between">
                ...
                <button
                  ...
                  className="w-8 h-8 rounded-xl bg-green-500/15 border border-green-500/25 flex items-center justify-center text-green-400"
                >
                  <Plus size={14} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-1">
                {convLoading && (
                  <p className="text-xs text-dark-500 text-center py-4">Loading…</p>
                )}
                {!convLoading && conversations.length === 0 && (
                  <p className="text-xs text-dark-500 text-center py-4">No conversations yet</p>
                )}
      ...
      <div className="flex items-center justify-between px-4 pt-14 pb-3 border-b border-white/5 shrink-0">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={handleNewConv}
            className="w-9 h-9 rounded-xl bg-dark-800 border border-white/8 flex items-center justify-center text-dark-400 hover:text-green-400 transition-colors"
      ...
      >
          ...
          >
            ...
            <div className="w-full max-w-sm space-y-2">
              {SUGGESTIONS.map((s, i) => (
                <motion.button
                  key={i}
                  onClick={() => handleSend(s.slice(2).trim())}
                  className="w-full text-left px-4 py-2.5 rounded-xl bg-dark-800 border border-white/8 text-xs text-dark-300 hover:text-white hover:border-white/15 transition-all font-body"
        ...
        {retryMsg && !loading && (
          ...
          >
            <button
              onClick={handleRetry}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-dark-800 border border-white/8 text-xs text-dark-400 hover:text-white transition-colors font-body"
            >
      ...
      <AnimatePresence>
        {showScrollBtn && (
          <motion.button
            className="fixed right-4 z-30 w-9 h-9 rounded-full bg-dark-800 border border-white/10 flex items-center justify-center text-dark-400 hover:text-white shadow-lg"
            style={{ bottom: '100px' }}
          ...
          <div className="flex-1 relative min-w-0">
            <textarea
              ...
              className="w-full max-w-full bg-dark-800 border border-white/10 rounded-2xl text-white font-body
                         ...
                         placeholder-dark-500 transition-all duration-200 disabled:opacity-50"
              style={{ boxSizing: 'border-box', minHeight: '48px', maxHeight: '140px' }}
            />
          </div>
          <motion.button
            onClick={() => handleSend()}
