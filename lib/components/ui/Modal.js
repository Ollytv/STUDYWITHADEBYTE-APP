...
function Modal({ isOpen, onClose, title, children, fullHeight = false }) {
    (0, react_1.useEffect)(() => {
        if (isOpen) {
    ...
    return (<framer_motion_1.AnimatePresence>
      {isOpen && (<>
          {/* Backdrop */}
          <framer_motion_1.motion.div className="fixed inset-0 z-[60] bg-black/70 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}/>
          ...
          {/* Sheet
            ...
            */}
          <framer_motion_1.motion.div className={`
              fixed bottom-0 z-[60]
            ...
            {/* Header */}
            {title && (<div className="flex items-center justify-between px-5 py-3 border-b border-white/5 shrink-0">
                <h2 className="text-lg font-display font-semibold text-white">{title}</h2>
                <button onClick={onClose} className="p-2 rounded-xl hover:bg-white/8 transition-colors text-dark-400 hover:text-white">
            ...
            */}
            <div className="flex-1 overflow-y-auto overflow-x-hidden overscroll-contain min-w-0 w-full" style={{ paddingBottom: 'env(safe-area-inset-bottom, 16px)' }}>
              {children}
            </div>
