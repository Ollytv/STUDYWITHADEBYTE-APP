...
const usePwaInstall_1 = require("../../hooks/usePwaInstall");
function CloseIcon() {
...
function CheckIcon() {
    return (<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
...
function InstallPrompt() {
    const { platform, shouldShow, canUseNativePrompt, promptInstall, dismiss } = (0, usePwaInstall_1.usePwaInstall)();
    const [dontShowAgain, setDontShowAgain] = (0, react_1.useState)(false);
    const [entered, setEntered] = (0, react_1.useState)(false);
    const [highlightSteps, setHighlightSteps] = (0, react_1.useState)(false);
    ...
    return (<div className={`fixed inset-0 z-[100] flex items-end sm:items-center justify-center transition-opacity duration-300 ${entered ? 'opacity-100' : 'opacity-0'}`} role="dialog" aria-modal="true" aria-labelledby="install-prompt-heading">
      {/* Backdrop */}
      ...
      <div className={`relative w-full sm:max-w-sm sm:mx-4 bg-dark-950 border border-white/10 rounded-t-3xl sm:rounded-3xl shadow-2xl transition-transform duration-300 ease-[cubic-bezier(0.23,1,0.32,1)] ${entered ? 'translate-y-0' : 'translate-y-full sm:translate-y-8'}`}>
        <button onClick={handleMaybeLater} aria-label="Close" className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full text-neutral-400 hover:text-white hover:bg-white/10 transition-colors no-select">
          <CloseIcon />
        </button>
        ...
        <div className="px-6 pt-7 pb-6 pb-safe max-h-[85vh] overflow-y-auto">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 id="install-prompt-heading" className="text-lg font-bold text-white leading-tight" style={{ fontFamily: "'Syne', sans-serif" }}>
                Install StudiByte
              </h2>
          ...
          <div className="grid grid-cols-2 gap-2.5 mb-6">
            {BENEFITS.map((benefit) => (<div key={benefit} className="flex items-center gap-2 glass rounded-xl px-2.5 py-2">
          ...
          {/* Steps */}
          <div className={`rounded-2xl border p-4 mb-6 transition-colors duration-300 ${highlightSteps ? 'border-green-500/60 bg-green-500/5' : 'border-white/10 bg-white/[0.03]'}`}>
            <p className="text-xs font-semibold text-neutral-500 uppercase tracking-wide mb-3">How to install</p>
            <ol className="space-y-3">
              {steps.map((step, i) => (<li key={i} className="flex items-start gap-3">
          ...
          <label className="flex items-center gap-2.5 mb-5 cursor-pointer no-select">
            <input type="checkbox" checked={dontShowAgain} onChange={(e) => setDontShowAgain(e.target.checked)} className="w-4 h-4 rounded accent-green-500 bg-white/10 border-white/20"/>
            <span className="text-xs text-neutral-400">Don&apos;t show this again</span>
          </label>

          {/* Actions */}
          <div className="flex flex-col gap-2.5">
            <button onClick={handleInstallNow} className="w-full py-3 rounded-xl bg-gradient-to-r from-green-400 to-emerald-500 text-dark-950 font-semibold text-sm hover:opacity-90 active:scale-[0.98] transition-all">
              Install Now
            </button>
            <button onClick={handleMaybeLater} className="w-full py-3 rounded-xl text-neutral-400 text-sm font-medium hover:text-white hover:bg-white/5 transition-colors">
              Maybe Later
            </button>
          </div>
        </div>
