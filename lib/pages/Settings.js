...
function Toggle({ value, onChange }) {
    return (<button onClick={() => onChange(!value)} className="relative flex-shrink-0 touch-manipulation" style={{ width: 48, height: 26, borderRadius: 13, background: value ? 'linear-gradient(135deg,#22c55e,#059669)' : 'rgba(255,255,255,0.1)', transition: 'background 0.2s', boxShadow: value ? '0 0 12px rgba(34,197,94,0.3)' : 'none' }}>
      <framer_motion_1.motion.div className="absolute rounded-full bg-white shadow-lg" style={{ width: 18, height: 18, top: 4 }} animate={{ left: value ? 26 : 4 }} transition={{ type: 'spring', stiffness: 500, damping: 30 }}/>
    </button>);
}
// ── Section wrapper ────────────────────────────────────────────────────────
function Section({ title, icon, children, accentColor = '#6b7280' }) {
    return (<framer_motion_1.motion.div className="rounded-2xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
...
function Row({ icon, label, sub, children, onClick, danger = false, iconBg }) {
    return (<framer_motion_1.motion.div className={`flex items-center gap-3 py-3.5 px-4 ${onClick ? 'cursor-pointer touch-manipulation' : ''}`} onClick={onClick} whileTap={onClick ? { scale: 0.98 } : undefined} style={onClick ? { WebkitTapHighlightColor: 'transparent' } : undefined}>
      <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: danger ? 'rgba(248,113,113,0.1)' : (iconBg || 'rgba(255,255,255,0.06)') }}>
      ...
      {children ?? (onClick && <lucide_react_1.ChevronRight size={15} style={{ color: '#374151' }}/>)}
    </framer_motion_1.motion.div>);
}
// ── Divider ────────────────────────────────────────────────────────────────
...
function PinDots({ length, filled }) {
    return (<div className="flex items-center justify-center gap-5 my-8">
      {Array.from({ length }).map((_, i) => (<framer_motion_1.motion.div key={i} className="rounded-full border-2" style={{ width: 18, height: 18, background: i < filled ? '#22c55e' : 'transparent', borderColor: i < filled ? '#22c55e' : '#374151', boxShadow: i < filled ? '0 0 8px rgba(34,197,94,0.5)' : 'none' }} animate={{ scale: i === filled - 1 ? [1, 1.4, 1] : 1 }} transition={{ duration: 0.15 }}/>))}
    </div>);
}
function PinPad({ onDigit, onDelete }) {
    const keys = ['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0', '⌫'];
    return (<div className="grid grid-cols-3 gap-3 px-4">
      {keys.map((k, i) => k === '' ? <div key={i}/> : (<framer_motion_1.motion.button key={k} onClick={() => k === '⌫' ? onDelete() : onDigit(k)} className="h-16 rounded-2xl text-xl font-black flex items-center justify-center touch-manipulation" style={{ background: k === '⌫' ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', color: k === '⌫' ? '#6b7280' : '#fff' }} whileTap={{ scale: 0.88 }}>{k}</framer_motion_1.motion.button>))}
    </div>);
}
// ── Legal / info modal ─────────────────────────────────────────────────────
function LegalModal({ isOpen, onClose, title, accentColor, downloadName, downloadContent, children, }) {
    return (<framer_motion_1.AnimatePresence>
      {isOpen && (<framer_motion_1.motion.div className="fixed inset-0 z-50 flex flex-col" style={{ background: 'rgba(0,0,0,0.85)' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <framer_motion_1.motion.div className="flex flex-col flex-1 overflow-hidden rounded-t-3xl mt-12" style={{ background: '#0a0a0f', border: '1px solid rgba(255,255,255,0.08)' }} initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', damping: 28, stiffness: 300 }}>
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
              ...
              <div className="flex items-center gap-2">
                <framer_motion_1.motion.button onClick={() => downloadAsText(downloadName, downloadContent)} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold touch-manipulation" style={{ background: `${accentColor}18`, border: `1px solid ${accentColor}40`, color: accentColor }} whileTap={{ scale: 0.93 }}>
                  <lucide_react_1.Download size={13}/> Download
                </framer_motion_1.motion.button>
                <framer_motion_1.motion.button onClick={onClose} className="w-9 h-9 rounded-xl flex items-center justify-center touch-manipulation" style={{ background: 'rgba(255,255,255,0.06)', color: '#6b7280' }} whileTap={{ scale: 0.9 }}>
                  <lucide_react_1.X size={17}/>
                </framer_motion_1.motion.button>
              </div>
            </div>
...
function LegalH2({ children, color = '#9ca3af' }) {
...
function LegalList({ items }) {
    return (<ul className="space-y-1.5 mt-1">
      {items.map((item, i) => (<li key={i} className="flex items-start gap-2 text-sm" style={{ color: '#9ca3af' }}>
...
function FAQItem({ q, a }) {
    ...
    return (<framer_motion_1.motion.div className="rounded-2xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
      <button className="w-full flex items-center justify-between px-4 py-3.5 text-left touch-manipulation" onClick={() => setOpen(o => !o)} style={{ WebkitTapHighlightColor: 'transparent' }}>
        <p className="text-sm font-bold text-white pr-4">{q}</p>
        {open ? <lucide_react_1.ChevronUp size={16} style={{ color: '#22c55e', flexShrink: 0 }}/> : <lucide_react_1.ChevronDown size={16} style={{ color: '#4b5563', flexShrink: 0 }}/>}
      </button>
      <framer_motion_1.AnimatePresence>
        {open && (<framer_motion_1.motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }} style={{ overflow: 'hidden' }}>
...
function ContactForm() {
    const [type, setType] = (0, react_1.useState)('support');
    ...
    if (sent)
        return (<framer_motion_1.motion.div className="rounded-2xl p-6 text-center" style={{ background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.25)' }} initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
      ...
      <button onClick={() => { setSent(false); setName(''); setEmail(''); setMessage(''); }} className="mt-4 text-xs font-bold text-green-400 touch-manipulation">Send another</button>
    </framer_motion_1.motion.div>);
    return (<div className="space-y-4">
      {/* Type selector */}
      <div className="grid grid-cols-3 gap-2">
        {[
            { id: 'support', icon: <lucide_react_1.Mail size={14}/>, label: 'Support' },
            { id: 'feedback', icon: <lucide_react_1.MessageSquare size={14}/>, label: 'Feedback' },
            { id: 'bug', icon: <lucide_react_1.Bug size={14}/>, label: 'Bug' },
        ].map(t => (<button key={t.id} onClick={() => setType(t.id)} className="flex flex-col items-center gap-1.5 py-3 rounded-2xl text-xs font-bold border transition-all touch-manipulation" style={type === t.id
      ...
      <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>
        <input value={name} onChange={e => setName(e.target.value)} placeholder="Your full name" className="w-full px-4 py-3.5 text-sm text-white focus:outline-none border-b" style={{ background: 'rgba(255,255,255,0.04)', borderColor: 'rgba(255,255,255,0.07)', color: '#fff' }}/>
        <input value={email} onChange={e => setEmail(e.target.value)} placeholder="Your email address" type="email" className="w-full px-4 py-3.5 text-sm text-white focus:outline-none border-b" style={{ background: 'rgba(255,255,255,0.04)', borderColor: 'rgba(255,255,255,0.07)', color: '#fff' }}/>
        <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder={type === 'bug' ? 'Describe the bug: what happened, when, and steps to reproduce...' : type === 'feedback' ? 'Share your thoughts, suggestions, or feature requests...' : 'How can we help you today?'} rows={5} className="w-full px-4 py-3.5 text-sm text-white focus:outline-none resize-none" style={{ background: 'rgba(255,255,255,0.04)', color: '#fff' }}/>
      </div>

      <framer_motion_1.motion.button onClick={handleSend} disabled={!name.trim() || !email.trim() || !message.trim()} className="w-full py-4 rounded-2xl font-black text-sm flex items-center justify-center gap-2 touch-manipulation" style={{ background: 'linear-gradient(135deg,#22c55e,#059669)', color: '#000', opacity: (!name.trim() || !email.trim() || !message.trim()) ? 0.4 : 1 }} whileTap={{ scale: 0.97 }}>
        <lucide_react_1.Send size={15}/> Send {type === 'support' ? 'Request' : type === 'feedback' ? 'Feedback' : 'Bug Report'}
      </framer_motion_1.motion.button>
...
// ══════════════════════════════════════════════════════════════════════════════
function Settings() {
    const { settings, updateSettings, profile, saveProfile, signOut } = (0, useStore_1.useStore)();
    // Modals
    const [modal, setModal] = (0, react_1.useState)(null);
    // Profile
    const [showProfile, setShowProfile] = (0, react_1.useState)(false);
    const [showLogoutConfirm, setShowLogoutConfirm] = (0, react_1.useState)(false);
    const [showPinSetup, setShowPinSetup] = (0, react_1.useState)(false);
    const [pinStep, setPinStep] = (0, react_1.useState)('set');
    const [pinFirst, setPinFirst] = (0, react_1.useState)('');
    const [pinInput, setPinInput] = (0, react_1.useState)('');
    const [pinError, setPinError] = (0, react_1.useState)('');
    const [profileForm, setProfileForm] = (0, react_1.useState)(profile || {
        fullName: '', department: '', programLevel: types_1.DEFAULT_PROGRAM_LEVEL,
        cgpaScale: types_1.DEFAULT_CGPA_SCALE, matricNumber: '',
        semesterStartDate: '', semesterEndDate: '', targetAttendance: 75,
        currentSemester: 'First',
        currentAcademicYear: `${new Date().getFullYear()}/${new Date().getFullYear() + 1}`,
        avatar: '',
    });
    const [notifGranted, setNotifGranted] = (0, react_1.useState)(Notification.permission === 'granted');
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            await saveProfile({
                ...
                programLevel: (profileForm.programLevel || types_1.DEFAULT_PROGRAM_LEVEL),
                cgpaScale: Number(profileForm.cgpaScale ?? types_1.DEFAULT_CGPA_SCALE),
                ...
                currentSemester: (profileForm.currentSemester || 'First'),
            ...
            const msg = e?.message || '';
            alert(msg.includes('too large') ? e.message :
    ...
    const handleExport = async () => {
        const data = await (0, db_1.exportAllData)();
...
A: Yes. All data is secured using Firebase with encrypted connections and strict security rules. Only you can access your data.`;
    return (<div className="min-h-screen bg-dark-950 pb-36 overflow-x-hidden">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative px-5 pt-14 pb-5">
          <framer_motion_1.motion.div className="flex items-center gap-2 mb-1" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <framer_motion_1.motion.h1 className="text-2xl font-black text-white" style={{ fontFamily: 'Georgia, serif', letterSpacing: '-0.5px' }} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.08 }}>
            Settings
      ...
      <div className="px-5 space-y-4">
        ...
        {/* ── PROFILE CARD ── */}
        <framer_motion_1.motion.button onClick={openProfile} className="w-full rounded-2xl p-4 text-left touch-manipulation relative overflow-hidden" style={{ background: 'linear-gradient(135deg,rgba(34,197,94,0.1),rgba(5,150,105,0.05))', border: '1px solid rgba(34,197,94,0.25)' }} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} whileTap={{ scale: 0.98 }}>
          ...
          <div className="flex items-center gap-4">
            ...
            ? <>
                    ...
                    {profile.matricNumber && (<span className="text-xs font-mono text-green-400 mt-1 inline-block px-2 py-0.5 rounded-lg" style={{ background: 'rgba(34,197,94,0.1)' }}>
            ...
            <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(34,197,94,0.1)' }}>
              <lucide_react_1.ChevronRight size={15} className="text-green-400"/>
            </div>
          </div>
        </framer_motion_1.motion.button>

        {/* ── APPEARANCE ── */}
        <Section title="Appearance" icon={<lucide_react_1.Sun size={12}/>} accentColor="#fbbf24">
          <Row icon={settings.theme === 'dark' ? <lucide_react_1.Moon size={16}/> : <lucide_react_1.Sun size={16}/>} label="Dark Mode" sub="Easier on the eyes at night" iconBg="rgba(251,191,36,0.1)">
            <Toggle value={settings.theme === 'dark'} onChange={v => updateSettings({ theme: v ? 'dark' : 'light' })}/>
          </Row>
        </Section>

        {/* ── NOTIFICATIONS ── */}
        <Section title="Notifications" icon={<lucide_react_1.Bell size={12}/>} accentColor="#60a5fa">
          <Row icon={<lucide_react_1.Bell size={16}/>} label="Enable Notifications" sub={notifGranted ? 'Class reminders are active' : 'Tap to enable class reminders'} iconBg="rgba(96,165,250,0.1)">
            {notifGranted
            ? <Toggle value={settings.notifications.enabled} onChange={v => updateSettings({ notifications: { ...settings.notifications, enabled: v } })}/>
            : <button onClick={handleEnableNotifs} className="text-xs font-bold px-3 py-1.5 rounded-xl touch-manipulation" style={{ background: 'rgba(96,165,250,0.12)', border: '1px solid rgba(96,165,250,0.25)', color: '#60a5fa' }}>Enable</button>}
          </Row>
          {settings.notifications.enabled && (<>
              {[['tenMinsBefore', '10 mins before'], ['thirtyMinsBefore', '30 mins before'], ['oneHourBefore', '1 hour before']].map(([key, label]) => (<div key={key}>
                  <Divider />
                  <Row icon={<lucide_react_1.Clock size={14}/>} label={label} iconBg="rgba(96,165,250,0.08)">
                    <Toggle value={settings.notifications[key]} onChange={v => updateSettings({ notifications: { ...settings.notifications, [key]: v } })}/>
                  </Row>
                </div>))}
              <Divider />
              <Row icon={<lucide_react_1.Volume2 size={14}/>} label="Sound" iconBg="rgba(96,165,250,0.08)">
        ...
        {/* ── SECURITY ── */}
        <Section title="Security" icon={<lucide_react_1.Shield size={12}/>} accentColor="#a78bfa">
          <Row icon={<lucide_react_1.Lock size={16}/>} label={hasPin ? 'Change PIN' : 'Set Up PIN'} sub={hasPin ? '4-digit PIN is active' : 'Add a PIN for extra security'} iconBg="rgba(167,139,250,0.1)" onClick={openPinSetup}/>
          {hasPin && (<><Divider /><Row icon={<lucide_react_1.Lock size={16}/>} label="Remove PIN" sub="Disable PIN protection" onClick={handleRemovePin} danger/></>)}
        </Section>

        {/* ── LEGAL & INFO ── */}
        <Section title="Legal & Information" icon={<lucide_react_1.FileText size={12}/>} accentColor="#34d399">
          {[
            { key: 'privacy', icon: <lucide_react_1.Shield size={16}/>, label: 'Privacy Policy', sub: 'How we handle your data', color: 'rgba(52,211,153,0.1)' },
            { key: 'terms', icon: <lucide_react_1.FileText size={16}/>, label: 'Terms & Conditions', sub: 'Rules for using StudiByte', color: 'rgba(96,165,250,0.1)' },
            { key: 'about', icon: <lucide_react_1.BookOpen size={16}/>, label: 'About Us', sub: 'The story behind StudiByte', color: 'rgba(251,191,36,0.1)' },
            { key: 'contact', icon: <lucide_react_1.Mail size={16}/>, label: 'Contact & Support', sub: 'Get help · Send feedback', color: 'rgba(167,139,250,0.1)' },
            { key: 'faq', icon: <lucide_react_1.HelpCircle size={16}/>, label: 'FAQ', sub: 'Frequently asked questions', color: 'rgba(251,146,60,0.1)' },
            { key: 'disclaimer', icon: <lucide_react_1.AlertTriangle size={16}/>, label: 'Disclaimer', sub: 'Important usage notices', color: 'rgba(248,113,113,0.1)' },
        ].map((item, i, arr) => (<div key={item.key}>
              {i > 0 && <Divider />}
              <Row icon={item.icon} label={item.label} sub={item.sub} iconBg={item.color} onClick={() => setModal(item.key)}/>
            </div>))}
        </Section>

        {/* ── DATA ── */}
        <Section title="Data & Storage" icon={<lucide_react_1.Database size={12}/>} accentColor="#34d399">
          <Row icon={<lucide_react_1.Download size={16}/>} label="Export Backup" sub="Download all your data as JSON" iconBg="rgba(52,211,153,0.1)" onClick={handleExport}/>
          <Divider />
          <Row icon={<lucide_react_1.Database size={16}/>} label="Cloud Storage" sub="Synced securely to Firebase" iconBg="rgba(52,211,153,0.08)"/>
        </Section>

        {/* ── ACCOUNT ── */}
        <Section title="Account" icon={<lucide_react_1.User size={12}/>} accentColor="#f87171">
          <Row icon={<lucide_react_1.LogOut size={16}/>} label="Log Out" sub="You'll be taken to the login screen." onClick={() => setShowLogoutConfirm(true)} danger/>
          <Divider />
          <Row icon={<lucide_react_1.Trash2 size={16}/>} label="Reset All Data" sub="Permanently deletes all records" onClick={handleReset} danger/>
        </Section>

        {/* ── ABOUT BRAND ── */}
        <framer_motion_1.motion.div className="rounded-2xl p-4 relative overflow-hidden" style={{ background: 'linear-gradient(135deg,rgba(34,197,94,0.08),rgba(5,150,105,0.04))', border: '1px solid rgba(34,197,94,0.15)' }} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <div className="absolute right-4 top-4 text-4xl opacity-10">🚀</div>
          <div className="flex items-center gap-2 mb-2">
            <lucide_react_1.Star size={11} className="text-green-400" fill="#22c55e"/>
      ...
      {/* PRIVACY POLICY */}
      <LegalModal isOpen={modal === 'privacy'} onClose={() => setModal(null)} title="Privacy Policy" accentColor="#34d399" downloadName="StudiByte-Privacy-Policy.txt" downloadContent={PRIVACY_TEXT}>
      ...
      {/* TERMS & CONDITIONS */}
      <LegalModal isOpen={modal === 'terms'} onClose={() => setModal(null)} title="Terms & Conditions" accentColor="#60a5fa" downloadName="StudiByte-Terms.txt" downloadContent={TERMS_TEXT}>
      ...
      {/* ABOUT US */}
      <LegalModal isOpen={modal === 'about'} onClose={() => setModal(null)} title="About Us" accentColor="#fbbf24" downloadName="StudiByte-About.txt" downloadContent={ABOUT_TEXT}>
      ...
      {/* CONTACT & SUPPORT */}
      <LegalModal isOpen={modal === 'contact'} onClose={() => setModal(null)} title="Contact & Support" accentColor="#a78bfa" downloadName="StudiByte-Contact.txt" downloadContent="StudiByte Support\nEmail: support@adebyte.tech\nResponse time: 24-48 hours\nADEBYTE TECH | Founded by Adegbite Olaoluwa">
        ...
        <div className="rounded-2xl p-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <div className="flex items-center gap-3 mb-2">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: 'rgba(34,197,94,0.1)' }}>
              <lucide_react_1.Mail size={14} className="text-green-400"/>
            </div>
            <div>
          ...
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: 'rgba(96,165,250,0.1)' }}>
              <lucide_react_1.ExternalLink size={14} className="text-blue-400"/>
            </div>
            <div>
      ...
      {/* FAQ */}
      <LegalModal isOpen={modal === 'faq'} onClose={() => setModal(null)} title="FAQ" accentColor="#fb923c" downloadName="StudiByte-FAQ.txt" downloadContent={FAQ_TEXT}>
      ...
      {/* DISCLAIMER */}
      <LegalModal isOpen={modal === 'disclaimer'} onClose={() => setModal(null)} title="Disclaimer" accentColor="#f87171" downloadName="StudiByte-Disclaimer.txt" downloadContent={DISCLAIMER_TEXT}>
        ...
        <LegalList items={['You are solely responsible for the accuracy of data you enter', 'You are responsible for all academic decisions made using this app', 'We are not liable for any academic penalties arising from reliance on app data', 'Always consult official academic resources for critical decisions']}/>
      ...
      {/* ── LOGOUT CONFIRM ── */}
      <Modal_1.Modal isOpen={showLogoutConfirm} onClose={() => setShowLogoutConfirm(false)} title="Log Out">
        <div className="p-5 space-y-5 pb-8">
          <div className="w-16 h-16 rounded-3xl flex items-center justify-center mx-auto" style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.2)' }}>
            <lucide_react_1.LogOut size={28} className="text-red-400"/>
          </div>
          ...
          <div className="flex gap-3">
            <Button_1.Button variant="secondary" fullWidth onClick={() => setShowLogoutConfirm(false)}>Cancel</Button_1.Button>
            <button onClick={handleLogout} className="flex-1 py-3 rounded-2xl font-bold text-sm touch-manipulation" style={{ background: 'linear-gradient(135deg,#ef4444,#dc2626)', color: '#fff' }}>Log Out</button>
          </div>
        </div>
      </Modal_1.Modal>

      {/* ── PIN SETUP ── */}
      <Modal_1.Modal isOpen={showPinSetup} onClose={() => setShowPinSetup(false)} title={pinStep === 'set' ? 'Set Your PIN' : 'Confirm PIN'}>
        <div className="pb-8">
          ...
          <framer_motion_1.AnimatePresence>
            {pinError && (<framer_motion_1.motion.p className="text-center text-xs text-red-400 font-medium mb-4" initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                {pinError}
              </framer_motion_1.motion.p>)}
          </framer_motion_1.AnimatePresence>
      ...
      {/* ── PROFILE MODAL ── */}
      <Modal_1.Modal isOpen={showProfile} onClose={() => setShowProfile(false)} title="Edit Profile" fullHeight>
        <div className="p-5 space-y-5 pb-10">
          <div className="flex flex-col items-center gap-3">
            <framer_motion_1.motion.div className="w-24 h-24 rounded-3xl overflow-hidden border-2 cursor-pointer relative group" style={{ borderColor: 'rgba(34,197,94,0.4)' }} onClick={() => avatarRef.current?.click()} whileTap={{ scale: 0.95 }}>
              {profileForm.avatar
            ...
            : <div className="w-full h-full flex items-center justify-center" style={{ background: 'linear-gradient(135deg,#22c55e,#065f46)' }}>
              ...
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" style={{ background: 'rgba(0,0,0,0.5)' }}>
                <lucide_react_1.Camera size={20} className="text-white"/>
              </div>
            </framer_motion_1.motion.div>
            <button onClick={() => avatarRef.current?.click()} className="text-xs font-bold px-3 py-1.5 rounded-xl touch-manipulation" style={{ border: '1px solid rgba(34,197,94,0.3)', color: '#4ade80' }}>
              Change Photo
            </button>
            <input ref={avatarRef} type="file" accept="image/*" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f)
        handleAvatarChange(f); }}/>
          </div>
          <Input_1.Input label="Full Name" value={profileForm.fullName || ''} onChange={v => setP('fullName', v)} placeholder="e.g. Adebayo Johnson" required/>
          <Input_1.Input label="Matric / Student ID" value={profileForm.matricNumber || ''} onChange={v => setP('matricNumber', v)} placeholder="e.g. STU/2023/001"/>
          <Input_1.Select label="Department" value={profileForm.department || ''} onChange={v => setP('department', v)} options={[{ value: '', label: 'Select department...' }, ...DEPARTMENTS.map(d => ({ value: d, label: d }))]}/>
          <Input_1.Select label="Academic Level" value={profileForm.programLevel || types_1.DEFAULT_PROGRAM_LEVEL} onChange={v => setP('programLevel', v)} options={[{ value: '', label: 'Select level...' }, ...LEVEL_OPTIONS]}/>
          <Input_1.Select label="School CGPA Scale" value={String(profileForm.cgpaScale ?? types_1.DEFAULT_CGPA_SCALE)} onChange={v => setP('cgpaScale', parseFloat(v))} options={types_1.CGPA_SCALE_OPTIONS.map(o => ({ value: String(o.value), label: o.label }))}/>
          <Input_1.Select label="Current Semester" value={profileForm.currentSemester || 'First'} onChange={v => setP('currentSemester', v)} options={[{ value: 'First', label: 'First Semester' }, { value: 'Second', label: 'Second Semester' }]}/>
          <div className="grid grid-cols-2 gap-3">
            <Input_1.Input label="Semester Start" type="date" value={profileForm.semesterStartDate || ''} onChange={v => setP('semesterStartDate', v)}/>
            <Input_1.Input label="Semester End" type="date" value={profileForm.semesterEndDate || ''} onChange={v => setP('semesterEndDate', v)}/>
          </div>
          <Input_1.Input label="Target Attendance %" type="number" value={String(profileForm.targetAttendance || 75)} onChange={v => setP('targetAttendance', parseInt(v) || 75)}/>
          <div className="flex gap-3 pt-2">
            <Button_1.Button variant="secondary" fullWidth onClick={() => setShowProfile(false)}>Cancel</Button_1.Button>
            <Button_1.Button fullWidth onClick={handleSaveProfile} disabled={saving || !profileForm.fullName?.trim()}>
