...
function Attendance() {
    const { classes, markAttendance, attendance, activeSemester, activeAcademicYear } = (0, useStore_1.useStore)();
    const [filter, setFilter] = (0, react_1.useState)('all');
    const [view, setView] = (0, react_1.useState)('courses');
    // Track which class is currently being marked to show loading state
    const [marking, setMarking] = (0, react_1.useState)(null);
    const handleMark = async (classId, attended) => {
        if (marking)
    ...
    const filteredClasses = (0, react_1.useMemo)(() => semClasses.filter(c => {
        if (filter === 'good')
            return c.attendancePercentage >= 75;
        if (filter === 'warning')
            return c.attendancePercentage >= 50 && c.attendancePercentage < 75;
        if (filter === 'danger')
            return c.attendancePercentage < 50;
        return true;
    }), [semClasses, filter]);
    ...
    const weeklyStats = (0, react_1.useMemo)(() => DAYS.map(day => {
    ...
    return (<div className="min-h-screen bg-dark-950 pb-28">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative px-5 pt-14 pb-4">
          <framer_motion_1.motion.div className="flex items-center gap-2 mb-1" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}>
            <lucide_react_1.Shield size={13} className="text-green-400"/>
          ...
          <framer_motion_1.motion.p className="text-xs text-dark-400 mt-0.5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }}>
      ...
      {/* ── SUMMARY HERO ────────────────────────────────────────────────── */}
      <framer_motion_1.motion.div className="mx-5 mb-5 rounded-2xl overflow-hidden relative" style={{ background: 'linear-gradient(135deg, #0f1f10, #1a2e1a)', border: '1px solid rgba(34,197,94,0.25)' }} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, type: 'spring', stiffness: 280 }}>

        {/* Decorative glow */}
        <div className="absolute -top-8 -right-8 w-32 h-32 rounded-full opacity-20" style={{ background: 'radial-gradient(circle, #22c55e, transparent)' }}/>

        <div className="p-5">
          <div className="flex items-center gap-5">
            <div className="relative">
              <ProgressRing_1.ProgressRing percentage={stats.overall} size={88} strokeWidth={7} color={ringColor}>
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-1.5 mt-2">
                {stats.overall >= 75
            ? <><lucide_react_1.TrendingUp size={11} className="text-green-400"/><span className="text-xs text-green-400 font-bold">On track ✓</span></>
            : <><lucide_react_1.AlertTriangle size={11} className="text-yellow-400"/><span className="text-xs text-yellow-400 font-bold">Needs improvement</span></>}
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 h-1.5 rounded-full bg-dark-700">
            <framer_motion_1.motion.div className="h-full rounded-full" style={{ background: `linear-gradient(90deg, ${ringColor}, ${ringColor}99)` }} initial={{ width: 0 }} animate={{ width: `${stats.overall}%` }} transition={{ delay: 0.4, duration: 0.8, ease: 'easeOut' }}/>
          </div>
        ...
        ].map((item, i) => (<div key={item.label} className={`py-3 text-center ${i < 2 ? 'border-r' : ''}`} style={{ background: item.bg, borderColor: 'rgba(255,255,255,0.06)' }}>
      ...
      <div className="flex items-center gap-3 px-5 mb-4">
        <div className="flex rounded-2xl p-1 gap-1" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.07)' }}>
          {[
            { id: 'courses', icon: <lucide_react_1.CheckCircle size={13}/>, label: 'Courses' },
            { id: 'weekly', icon: <lucide_react_1.BarChart2 size={13}/>, label: 'Weekly' },
        ].map(({ id, icon, label }) => (<button key={id} onClick={() => setView(id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all touch-manipulation" style={view === id
                ? { background: 'linear-gradient(135deg, #22c55e, #059669)', color: '#000' }
                : { color: '#6b7280' }}>
              {icon}{label}
            </button>))}
        </div>
        {view === 'courses' && (<div className="flex gap-1.5 overflow-x-auto scrollbar-hide flex-1">
            {[['all', 'All'], ['good', '≥75%'], ['warning', 'Warn'], ['danger', 'Risk']].map(([id, label]) => (<button key={id} onClick={() => setFilter(id)} className="shrink-0 px-2.5 py-1.5 rounded-xl text-xs font-bold border transition-all touch-manipulation" style={filter === id
      ...
      <framer_motion_1.AnimatePresence mode="wait">
        {view === 'weekly' ? (<framer_motion_1.motion.div key="weekly" className="px-5 space-y-2.5" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            <div className="rounded-2xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
              ...
              <div className="px-5 pb-5 space-y-4">
                ...
                return (<framer_motion_1.motion.div key={d.day} initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}>
                      ...
                      <div className="h-2.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
                        <framer_motion_1.motion.div className="h-full rounded-full" style={{ background: `linear-gradient(90deg, ${color}, ${color}88)` }} initial={{ width: 0 }} animate={{ width: `${d.pct}%` }} transition={{ duration: 0.7, ease: 'easeOut', delay: i * 0.06 }}/>
                      </div>
                    </framer_motion_1.motion.div>);
            })}
                {weeklyStats.every(d => d.count === 0) && (<p className="text-sm text-dark-500 text-center py-8">No data yet — start marking attendance</p>)}
              </div>
            </div>
          </framer_motion_1.motion.div>) : (<framer_motion_1.motion.div key="courses" className="px-5 space-y-3" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
            {filteredClasses.length === 0 ? (<EmptyState_1.EmptyState icon={<lucide_react_1.CheckCircle size={28}/>} title="No courses found" description="Attendance records appear once you start tracking."/>) : (filteredClasses.map((cls, i) => {
                ...
                return (<framer_motion_1.motion.div key={cls.id} className="rounded-2xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05, type: 'spring', stiffness: 300 }}>
                    ...
                    <div className="p-4">
                      ...
                      <div className="mb-3">
                        ...
                        <div className="h-2 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
                          <framer_motion_1.motion.div className="h-full rounded-full" style={{ background: `linear-gradient(90deg, ${pctColor}, ${pctColor}77)` }} initial={{ width: 0 }} animate={{ width: `${cls.attendancePercentage}%` }} transition={{ duration: 0.7, ease: 'easeOut', delay: i * 0.05 }}/>
                        </div>
                      </div>
                      ...
                      {/* Warning banner */}
                      {cls.totalClassesHeld > 0 && cls.attendancePercentage < 75 && needed75 > 0 && (<framer_motion_1.motion.div className="flex items-center gap-2 py-2 px-3 rounded-xl mb-3" style={{ background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.2)' }} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
                          <lucide_react_1.AlertTriangle size={11} className="text-yellow-400 shrink-0"/>
                      ...
                      <div className="flex gap-2">
                        <framer_motion_1.motion.button onClick={() => handleMark(cls.id, true)} disabled={marking === `${cls.id}-true`} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-bold touch-manipulation" style={{
                        ...
                        opacity: marking === `${cls.id}-true` ? 0.6 : 1,
                          ...
                          {marking === `${cls.id}-true`
                        ? <framer_motion_1.motion.div className="w-3 h-3 border-2 border-green-400 border-t-transparent rounded-full" animate={{ rotate: 360 }} transition={{ duration: 0.6, repeat: Infinity, ease: 'linear' }}/>
                        : <lucide_react_1.CheckCircle size={13}/>}
                          {marking === `${cls.id}-true` ? 'Saving…' : 'Attended'}
                        </framer_motion_1.motion.button>
                        <framer_motion_1.motion.button onClick={() => handleMark(cls.id, false)} disabled={marking === `${cls.id}-false`} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-bold touch-manipulation" style={{
                        ...
                        ? <framer_motion_1.motion.div className="w-3 h-3 border-2 border-red-400 border-t-transparent rounded-full" animate={{ rotate: 360 }} transition={{ duration: 0.6, repeat: Infinity, ease: 'linear' }}/>
                        : <lucide_react_1.XCircle size={13}/>}
