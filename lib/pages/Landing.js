...
function Section({ children, className = '', id = '' }) {
    return (<section id={id} className={`px-5 py-16 max-w-2xl mx-auto ${className}`}>
      {children}
    </section>);
}
...
function FAQItem({ q, a, index }) {
    ...
    return (<framer_motion_1.motion.div className="border-b border-white/6" initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.03 }}>
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between py-4 text-left gap-3 touch-manipulation">
        <span className="text-sm font-semibold text-white font-body leading-snug">{q}</span>
        <framer_motion_1.motion.div animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.25 }} className="flex-shrink-0">
          <lucide_react_1.ChevronDown size={15} className="text-dark-500"/>
        </framer_motion_1.motion.div>
      </button>
      <framer_motion_1.AnimatePresence initial={false}>
        {open && (<framer_motion_1.motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] }} className="overflow-hidden">
...
function FeatureCard({ icon: Icon, title, desc, color, delay = 0, }) {
    return (<framer_motion_1.motion.div className="p-5 rounded-2xl border border-white/6 bg-dark-900/60" style={{ backdropFilter: 'blur(12px)' }} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay, duration: 0.5, ease: [0.22, 1, 0.36, 1] }}>
...
// ── Problem card ──────────────────────────────────────────────────────────────
function ProblemCard({ emoji, title, desc, delay = 0 }) {
    return (<framer_motion_1.motion.div className="flex gap-3 p-4 rounded-2xl bg-dark-900/50 border border-white/5" initial={{ opacity: 0, x: -12 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay, duration: 0.45 }}>
...
// ── Why card ──────────────────────────────────────────────────────────────────
function WhyCard({ icon: Icon, title, desc, color, delay = 0 }) {
    return (<framer_motion_1.motion.div className="flex gap-3 items-start" initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay }}>
      <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: `${color}15`, border: `1px solid ${color}25` }}>
...
function Landing({ onGetStarted }) {
    ...
    return (<div className="min-h-screen bg-dark-950 overflow-x-hidden">
      ...
      {/* ── NAV ─────────────────────────────────────────────────────────── */}
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-5" style={{
        ...
        <button onClick={onGetStarted} className="text-xs font-bold text-dark-950 bg-green-500 px-4 py-2 rounded-xl touch-manipulation active:scale-95 transition-transform">
          Get Started
        </button>
      </nav>

      {/* ── HERO ────────────────────────────────────────────────────────── */}
      <section className="relative min-h-screen flex flex-col justify-center px-5 pb-16 max-w-2xl mx-auto overflow-hidden" style={{ paddingTop: 'calc(env(safe-area-inset-top, 0px) + 96px)' }}>
        {/* Background glow */}
        <div className="absolute -top-20 -right-20 w-72 h-72 rounded-full opacity-10 pointer-events-none" style={{ background: 'radial-gradient(circle, #22c55e, transparent 70%)' }}/>
        <div className="absolute bottom-0 -left-20 w-56 h-56 rounded-full opacity-8 pointer-events-none" style={{ background: 'radial-gradient(circle, #a855f7, transparent 70%)' }}/>

        <framer_motion_1.motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0}>
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full mb-6" style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.2)' }}>
            <lucide_react_1.Sparkles size={11} className="text-green-400"/>
            <span className="text-[10px] font-bold text-green-400 uppercase tracking-widest">AI-Powered • Free to Start</span>
          </div>
        </framer_motion_1.motion.div>

        <framer_motion_1.motion.h1 className="text-[2.4rem] font-black text-white leading-[1.1] mb-4 font-display" style={{ letterSpacing: '-1.5px' }} variants={fadeUp} initial="hidden" animate="visible" custom={1}>
          Study smarter.<br />
          <span className="text-green-400">Track everything.</span><br />
          Graduate stronger.
        </framer_motion_1.motion.h1>

        <framer_motion_1.motion.p className="text-sm text-dark-300 font-body leading-relaxed mb-8 max-w-sm" variants={fadeUp} initial="hidden" animate="visible" custom={2}>
        ...
        <framer_motion_1.motion.p className="text-sm text-dark-300 font-body leading-relaxed mb-8" variants={fadeUp} initial="hidden" animate="visible" custom={2}>
        ...
        <framer_motion_1.motion.div className="flex flex-col gap-3 max-w-xs" variants={fadeUp} initial="hidden" animate="visible" custom={3}>
          <button onClick={onGetStarted} className="flex items-center justify-center gap-2 py-3.5 rounded-2xl font-bold text-sm text-dark-950 touch-manipulation active:scale-[0.97] transition-transform" style={{ background: 'linear-gradient(135deg, #22c55e, #16a34a)' }}>
            Start for Free <lucide_react_1.ArrowRight size={15}/>
          </button>
          <button onClick={() => document.getElementById('features')?.scrollIntoView({ behavior: 'smooth' })} className="flex items-center justify-center gap-2 py-3.5 rounded-2xl font-semibold text-sm text-dark-300 border border-white/10 touch-manipulation">
            See what's inside <lucide_react_1.ChevronDown size={14}/>
          </button>
        </framer_motion_1.motion.div>

        {/* Floating stats */}
        <framer_motion_1.motion.div className="flex gap-3 mt-10" variants={fadeUp} initial="hidden" animate="visible" custom={4}>
          {[
            { label: 'Features', value: '10+' },
            { label: 'GPA Scales', value: '3.0 – 5.0' },
            { label: 'AI Powered', value: '✓' },
        ].map(s => (<div key={s.label} className="px-3 py-2 rounded-xl bg-dark-800/60 border border-white/6 text-center">
      ...
      <Section id="problems">
        ...
        <framer_motion_1.motion.h2 className="text-2xl font-black text-white font-display mb-2" style={{ letterSpacing: '-0.8px' }} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          Every student faces these.
        </framer_motion_1.motion.h2>
        <framer_motion_1.motion.p className="text-sm text-dark-400 font-body mb-8 leading-relaxed" variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={1}>
      ...
      <Section>
        ...
        ].map((item, i) => (<framer_motion_1.motion.div key={item.step} className="flex gap-4" initial={{ opacity: 0, x: -12 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }}>
      ...
      <Section id="features">
        ...
        <div className="grid grid-cols-2 gap-3">
          <FeatureCard icon={lucide_react_1.Brain} color="#a855f7" delay={0} title="AI Study Assistant" desc="Ask questions on any subject and get clear, structured explanations instead of vague search results. The AI summarises long notes into key points, generates practice quizzes to test yourself before an exam, and walks through worked examples step by step — useful any time you're stuck with no lecturer around to ask."/>
          <FeatureCard icon={lucide_react_1.TrendingUp} color="#fbbf24" delay={0.05} title="GPA Tracker" desc="Enter your scores as results come in and StudiByte calculates your GPA and CGPA instantly, using your exact school grading scale — 3.0, 4.0, or 5.0. Instead of waiting for an official transcript to find out where you stand, see in real time which courses are helping or hurting your average."/>
          <FeatureCard icon={lucide_react_1.ClipboardList} color="#c084fc" delay={0.1} title="Assignment Manager" desc="Add every assignment with its deadline, course, and priority level the moment it's announced. StudiByte keeps the most urgent tasks visible at the top and reminds you before they're due, so important work never quietly slips past while you're focused on something else. Mark items complete as you finish."/>
          <FeatureCard icon={lucide_react_1.Timer} color="#fb923c" delay={0.15} title="Study Timer" desc="Use the built-in Pomodoro-style timer to study in focused intervals with structured breaks in between, which research shows improves concentration and retention compared to long, unbroken sessions. StudiByte tracks your total study time per session and across the semester, so you can see your real effort, not just your intentions."/>
          <FeatureCard icon={lucide_react_1.BookOpen} color="#22d3ee" delay={0.2} title="Course Materials" desc="Upload PDFs, scanned notes, images, and documents for each course as soon as you receive them. Materials are automatically organised by semester and course, so instead of scrolling through chats or folders during exam week, you open StudiByte and find exactly the file you need in seconds."/>
          <FeatureCard icon={lucide_react_1.Bell} color="#f87171" delay={0.25} title="Class Reminders" desc="Get a push notification before each class starts — 10 minutes, 30 minutes, or a full hour ahead, whichever suits you. Reminders only fire for classes actually scheduled that day, so you're never caught off guard by a lecture you forgot was happening, even during a hectic week."/>
          <FeatureCard icon={lucide_react_1.CheckCircle} color="#34d399" delay={0.3} title="Attendance Tracking" desc="Mark each class as attended or missed with a single tap. StudiByte calculates your attendance percentage per course in real time and warns you as you approach your school's minimum threshold, so you can correct course early instead of discovering the problem only when you're barred from an exam."/>
          <FeatureCard icon={lucide_react_1.Cloud} color="#60a5fa" delay={0.35} title="Cloud Sync" desc="Every timetable entry, grade, assignment, and uploaded file is securely stored and synced across your devices automatically. Start an assignment on your laptop, check tomorrow's classes on your phone on the bus, and everything stays exactly up to date — no manual exporting or emailing files to yourself."/>
          <FeatureCard icon={lucide_react_1.Search} color="#e879f9" delay={0.4} title="Smart Timetable" desc="Build your full weekly class schedule once and see today's classes at a glance the moment you open the app. StudiByte can also read an existing timetable from a PDF or screenshot using AI-powered document parsing, so you don't have to retype a schedule you already have written down somewhere."/>
          <FeatureCard icon={lucide_react_1.Smartphone} color="#4ade80" delay={0.45} title="Install as App" desc="StudiByte installs directly to your home screen like a native app — no app store, no separate download. Once installed, it loads faster, works offline for your timetable and GPA data, and sends push notifications for class reminders, all without using up your phone's storage space."/>
        </div>
      </Section>
      ...
      <Section id="ai">
        ...
        <framer_motion_1.motion.p className="text-sm text-dark-400 font-body mb-6 leading-relaxed" variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={1}>
        ...
        {/* Mock chat UI */}
        <framer_motion_1.motion.div className="rounded-2xl border border-white/8 bg-dark-900 overflow-hidden mb-6" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          <div className="flex items-center gap-2 px-4 py-3 border-b border-white/6">
            <div className="w-6 h-6 rounded-lg bg-purple-500/20 border border-purple-500/30 flex items-center justify-center">
              <lucide_react_1.Brain size={12} className="text-purple-400"/>
            </div>
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="flex gap-2">
              <div className="w-6 h-6 rounded-xl bg-purple-500/20 border border-purple-500/30 flex items-center justify-center flex-shrink-0 mt-0.5">
                <lucide_react_1.Brain size={11} className="text-purple-400"/>
              </div>
        ...
        <div className="space-y-3">
          {[
            { icon: lucide_react_1.Brain, color: '#a855f7', title: 'Concept explanations', desc: 'Breaks down complex topics from any subject into clear, digestible explanations with examples.' },
            { icon: lucide_react_1.FileText, color: '#22d3ee', title: 'Note summarisation', desc: 'Paste your lecture notes and the AI produces a concise, exam-ready summary in seconds.' },
            { icon: lucide_react_1.Award, color: '#fbbf24', title: 'Practice quiz generation', desc: 'Ask it to generate test questions from any topic so you can test yourself before exams.' },
            { icon: lucide_react_1.Zap, color: '#34d399', title: 'Instant revision help', desc: 'Review entire topics quickly by asking the AI to walk you through the most important points.' },
        ].map((item, i) => (<WhyCard key={item.title} {...item} delay={i * 0.06}/>))}
        </div>

        <framer_motion_1.motion.div className="mt-5 p-4 rounded-2xl" style={{ background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.15)' }} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
      ...
      <Section id="resources">
        ...
        ].map((item, i) => (<framer_motion_1.motion.div key={item.title} className="p-4 rounded-2xl bg-dark-900/50 border border-white/5" initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }}>
      ...
      {/* ── INSTALL ──────────────────────────────────────────────────────── */}
        ...
        <framer_motion_1.motion.p className="text-sm text-dark-400 font-body mb-7 leading-relaxed" variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={1}>
          ...
          {[
            ...
            },
            {
        ...
        ].map((platform, i) => (<framer_motion_1.motion.div key={platform.label} className="p-4 rounded-2xl bg-dark-900/60 border border-white/6" initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-7 h-7 rounded-xl flex items-center justify-center" style={{ background: `${platform.color}15`, border: `1px solid ${platform.color}25` }}>
              ...
              <ol className="space-y-1.5">
                {platform.steps.map((step, si) => (<li key={si} className="flex gap-2 text-xs text-dark-400 font-body">
      ...
      <Section>
        ...
        <framer_motion_1.motion.h2 className="text-2xl font-black text-white font-display mb-8" style={{ letterSpacing: '-0.8px' }} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          Designed for results, not just features.
        </framer_motion_1.motion.h2>
        <div className="space-y-5">
          {[
            { icon: lucide_react_1.Zap, color: '#fbbf24', title: 'Everything in one place', desc: 'Stop switching between five apps to manage your academic life. StudiByte brings your timetable, GPA, assignments, materials, and AI tutor into one focused workspace.' },
            { icon: lucide_react_1.Shield, color: '#34d399', title: 'Your data stays private', desc: 'StudiByte uses Firebase Authentication and Firestore security rules. Your academic data is private to your account and never shared or sold.' },
            { icon: lucide_react_1.Smartphone, color: '#60a5fa', title: 'Built mobile-first', desc: 'Designed for the device you actually use — your phone. Every screen, button, and interaction is optimised for one-handed mobile use.' },
            { icon: lucide_react_1.Brain, color: '#a855f7', title: 'AI that actually helps', desc: 'Not a gimmick. The AI assistant answers real academic questions across all subjects and improves the more context you give it.' },
            { icon: lucide_react_1.Award, color: '#f97316', title: 'Real GPA visibility', desc: 'Know where you stand at all times. Enter scores as results come in and watch your GPA update in real time — no more waiting for the end-of-semester shock.' },
            { icon: lucide_react_1.Users, color: '#e879f9', title: 'Works for any institution', desc: 'Whether your school uses a 3.0, 4.0, or 5.0 CGPA scale, StudiByte adapts to your grading system automatically.' },
            { icon: lucide_react_1.CheckCircle, color: '#22c55e', title: 'Attendance you can trust', desc: 'Track every class with one tap. Know exactly how many classes you can miss before it affects your grade — before it happens, not after.' },
            { icon: lucide_react_1.Download, color: '#94a3b8', title: 'Works without internet', desc: 'Core features like timetable and GPA data are available offline. Changes sync automatically when you reconnect.' },
        ].map((item, i) => (<WhyCard key={item.title} {...item} delay={i * 0.05}/>))}
        </div>
      </Section>
      ...
      <Section id="about">
        ...
        <framer_motion_1.motion.h2 className="text-2xl font-black text-white font-display mb-4" style={{ letterSpacing: '-0.8px' }} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
        ...
        ].map((item, i) => (<framer_motion_1.motion.div key={item.title} className="p-4 rounded-2xl bg-dark-900/50 border border-white/5" initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }}>
              <p className="text-xs font-black text-green-400 uppercase tracking-wider mb-1.5">{item.title}</p>
              <p className="text-sm text-dark-300 font-body leading-relaxed">{item.desc}</p>
            </framer_motion_1.motion.div>))}
        </div>
      ...
      <Section id="faq">
        ...
        <framer_motion_1.motion.h2 className="text-2xl font-black text-white font-display mb-6" style={{ letterSpacing: '-0.8px' }} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
      ...
      <section className="px-5 py-16 max-w-2xl mx-auto">
        <framer_motion_1.motion.div className="relative rounded-3xl p-8 text-center overflow-hidden" style={{ background: 'linear-gradient(135deg, rgba(34,197,94,0.12), rgba(168,85,247,0.08))', border: '1px solid rgba(34,197,94,0.2)' }} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <div className="absolute -top-8 -right-8 w-32 h-32 rounded-full opacity-10 pointer-events-none" style={{ background: 'radial-gradient(circle, #22c55e, transparent)' }}/>
          <div className="w-12 h-12 rounded-2xl bg-green-500/15 border border-green-500/25 flex items-center justify-center mx-auto mb-4">
            <lucide_react_1.Sparkles size={20} className="text-green-400"/>
          </div>
          ...
          <button onClick={onGetStarted} className="inline-flex items-center gap-2 px-6 py-3.5 rounded-2xl font-bold text-sm text-dark-950 touch-manipulation active:scale-[0.97] transition-transform" style={{ background: 'linear-gradient(135deg, #22c55e, #16a34a)' }}>
            Create Free Account <lucide_react_1.ArrowRight size={15}/>
          </button>
      ...
      <footer className="px-5 py-8 max-w-2xl mx-auto border-t border-white/5">
        ...
        <div className="flex gap-4 mt-4">
          {['Features', 'AI Assistant', 'Install', 'About', 'FAQ'].map(link => (<button key={link} onClick={() => document.getElementById(link.toLowerCase().replace(' ', ''))?.scrollIntoView({ behavior: 'smooth' })} className="text-[10px] text-dark-500 hover:text-dark-300 transition-colors font-body">
              {link}
            </button>))}
        </div>
      </footer>
      ...
      <framer_motion_1.AnimatePresence>
        {showScrollBtn && (<framer_motion_1.motion.button onClick={scrollToBottom} className="fixed bottom-6 right-5 z-50 w-10 h-10 rounded-full flex items-center justify-center shadow-lg" style={{
