...
function Onboarding() {
    ...
    const finish = async () => {
        ...
        try {
            ...
            (0, notifications_1.requestNotificationPermission)().catch(() => { });
            ...
            await updateSettings({ onboardingComplete: true, firstLaunch: false }).catch(() => { });
        }
        finally {
            setSaving(false);
        }
    ...
    if (profileStep) {
        return (<framer_motion_1.motion.div className="min-h-screen bg-dark-950 flex flex-col px-6 pt-14 pb-safe" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ type: 'spring', damping: 28, stiffness: 300 }}>
        <div className="mb-8">
          <div className="w-12 h-12 rounded-2xl bg-green-500/15 border border-green-500/25 flex items-center justify-center mb-5">
            <lucide_react_1.Sparkles size={22} className="text-green-400"/>
          </div>
        ...
        <div className="flex-1 space-y-5">
          <Input_1.Input label="Full Name" value={fullName} onChange={v => { setFullName(v); setFormError(''); }} placeholder="e.g. Adebayo Johnson"/>
          <Input_1.Select label="Department" value={department} onChange={v => { setDepartment(v); setFormError(''); }} options={[
                { value: '', label: 'Select your department...' },
                ...DEPARTMENTS.map(d => ({ value: d, label: d })),
            ]}/>
          <Input_1.Select label="Academic Level" value={programLevel} onChange={v => setProgramLevel(v)} options={LEVEL_OPTIONS}/>
        </div>

        <div className="space-y-3 mt-8 pb-8">
          {/* Inline validation error */}
          {formError && (<framer_motion_1.motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="text-xs text-red-400 font-body text-center pb-1">
              {formError}
            </framer_motion_1.motion.p>)}

          {/* Let's go — requires name + department */}
          <button onClick={finish} disabled={saving} className="w-full py-4 rounded-2xl bg-green-500 text-dark-950 font-display font-bold text-base
                       disabled:opacity-50 transition-opacity touch-manipulation active:scale-[0.98]" style={{ WebkitTapHighlightColor: 'transparent' }}>
    ...
    // ── Intro slides ───────────────────────────────────────────────────────────
    return (<div className="min-h-screen bg-dark-950 flex flex-col overflow-hidden">
      ...
      <div className="relative flex flex-col min-h-screen px-6 pt-14 pb-safe">
        ...
        <button onClick={() => setProfileStep(true)} className="self-end mb-4 px-4 py-2 text-sm font-body text-dark-400 hover:text-white transition-colors touch-manipulation" style={{ WebkitTapHighlightColor: 'transparent', minWidth: 60, minHeight: 44 }}>
          Skip
        </button>
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          <framer_motion_1.AnimatePresence mode="wait">
            <framer_motion_1.motion.div key={step} initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -20, scale: 0.95 }} transition={{ type: 'spring', damping: 30, stiffness: 300 }} className="flex flex-col items-center">
              <framer_motion_1.motion.div className="w-24 h-24 rounded-3xl border-2 flex items-center justify-center mb-8" style={{ background: slide.iconBg, borderColor: slide.iconBorder }} animate={{ y: [0, -8, 0] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}>
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          {slides.map((_, i) => (<framer_motion_1.motion.div key={i} onClick={() => setStep(i)} className="rounded-full cursor-pointer" animate={{ width: i === step ? 24 : 8, height: 8, background: i === step ? '#22c55e' : '#333' }} transition={{ duration: 0.3 }}/>))}
        </div>

        <button onClick={() => isLast ? setProfileStep(true) : setStep(s => s + 1)} className="w-full py-4 rounded-2xl bg-green-500 text-dark-950 font-display font-bold text-base
                     flex items-center justify-center gap-2 touch-manipulation active:scale-[0.98] transition-transform" style={{ WebkitTapHighlightColor: 'transparent' }}>
          {isLast ? 'Get Started 🎉' : 'Next'}
          {!isLast && <lucide_react_1.ArrowRight size={18}/>}
        </button>
        <div className="h-8"/>
      </div>
    </div>);
